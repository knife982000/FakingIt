...
class TwitterCrawler(val storage: MongoDBStorage): AutoCloseable {
	...
	fun run(tweetIds : List<Long>,recursive : Boolean=false,downloadReplies : Boolean=false, downloadQuotes : Boolean=false) {
		...
		if(downloadQuotes){
			tweetIdsFiltered = tweetIds.toMutableList()
					...
					tweetIdsFiltered.asSequence().filter{this.storage.findTweet(it) != null && this.storage.findTweet(it)!!.userId > 0}
			.chunked(99)
			.forEach{this.tweetQuoteDownload(it)}
		}
	...
	private fun twitterCrawl(){ 
		var currentQuery = this.storage.nextQueryDownload()
				...
				while(currentQuery != null){
					...
					currentQuery = this.storage.nextQueryDownload()
				}
